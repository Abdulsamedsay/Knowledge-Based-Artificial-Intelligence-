...
def get_players(game_n: int) -> List[PlayerController]:
    ...
    """
    #We defined heuristics for both players 
    heuristic1: Heuristic = SimpleHeuristic(game_n)
    heuristic2: Heuristic = SimpleHeuristic(game_n)
    #Create different players with different stategies 
    # - p1: uses minimax search with a given depth and heuristic
    # - p2: uses alpha-beta pruning with the same depth and heuristic
    p1: PlayerController = MinMaxPlayer(1, game_n, depth=4, heuristic = heuristic1)
    p2: PlayerController = AlphaBetaPlayer(2, game_n, depth=4, heuristic = heuristic2)
